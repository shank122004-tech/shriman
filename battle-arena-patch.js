  ...
  */
  ...
  const _syncCoinsToFirebase = async (userUid, coinsToAdd, reason) => {
    ...
    try {
      ...
      current.coins = Math.max(0, (current.coins || 0) + coinsToAdd);
      ...
      if (typeof refreshAllCoinsDisplay === 'function') {
  ...
  window.BA = {
    ...
    close() {
      try {
        // ✅ If the "upgrade to create battles" premium modal is still open
        // (from _openBattlePlanModal), closing the arena should close it
        // too — otherwise it's left sitting on top of everything (z-index
        // 999999) with nothing left to dismiss it, which looks like "the
        // premium page randomly showing" right after closing the arena.
        if (this._premiumOpenedByBattleGate) {
          if (this._planModalObserver) {
            this._planModalObserver.disconnect();
            this._planModalObserver = null;
          }
          this._premiumOpenedByBattleGate = false;
          const premiumModal = document.getElementById('premiumModal');
          if (premiumModal && premiumModal.classList.contains('active')) {
            if (typeof closePremiumModal === 'function') closePremiumModal();
            else premiumModal.classList.remove('active');
          }
        }
    ...
    _openBattlePlanModal() {
      ...
      this._premiumOpenedByBattleGate = true;
      const observer = new MutationObserver(() => {
        const m = document.getElementById('premiumModal');
        if (m && !m.classList.contains('active')) {
          BA._premiumOpenedByBattleGate = false;
          BA._planModalObserver = null;
          const baModal = document.getElementById('ba-modal');
          if (baModal) {
      ...
      this._planModalObserver = observer;
      const premiumModal = document.getElementById('premiumModal');
      if (premiumModal) {
    ...
    async _createBattle() {
      ...
      try {
        ...
        if (!validationResponse.ok) {
          ...
          // ✅ Distinguish WHY battle creation was blocked, so free users get
          // pointed at "upgrade to create battles" and users who used up
          // their paid plan's monthly battles get a clear "limit reached"
          // message — both routed to the same premium modal, scrolled to
          // the Battle Creator plan section. Every other error (bad auth,
          // rate limit, server error, cross-user block) keeps the original
          // generic behavior untouched.
          if (validationResponse.status === 403 && typeof errorData.allowedBattles === 'number') {
            if (errorData.allowedBattles === 0) {
              // Free plan — battles aren't included at all.
              toast('⚔️ Battle Creator is a premium feature. Upgrade your plan to create battles.', 4500);
            } else {
              // Paid plan, but this month's battle quota is used up.
              toast(`⚔️ You've used all ${errorData.allowedBattles} battles on your current plan this month. Upgrade to create more.`, 4500);
            }
...
(function () {
  ...
  function _syncCoinsToFirestore(total) {
    try {
      ...
      const u   = window._firebaseAuth?.currentUser;
      if (!db || !fns || !u) return Promise.resolve();
      
      const { doc, updateDoc } = fns;
      // Use updateDoc for better merge behavior - updates only the coins field
      return updateDoc(doc(db, 'userCoins', u.uid), { 
        coins: total, 
        updatedAt: Date.now() 
      }).catch(() => {});
    } catch (_) {
  ...
  function _loadCoinsFromFirestore() {
    try {
      ...
      return getDoc(doc(db, 'userCoins', u.uid)).then(snap => {
        if (snap && snap.exists()) {
  ...
  function addCoins(n, reason) {
    ...
    try {
      if (typeof window.refreshCoinUI === 'function') window.refreshCoinUI();
    ...
    // 4. Sync to Firestore in background and reload global coins
    _syncCoinsToFirestore(newTotal).then(() => {
      // After sync completes, try to sync from global app.js if available
      try {
        if (typeof window.loadCoinsFromFirebase === 'function') {
          const uid = window._firebaseAuth?.currentUser?.uid;
          if (uid) window.loadCoinsFromFirebase(uid);
        }
      } catch(e) {}
    });
...
(function() {
  ...
  // ✅ SYNC COINS ON PAGE LOAD: Load coins from Firestore to keep them in sync across devices
  (function initCoinSync() {
    try {
      const uid = window._firebaseAuth?.currentUser?.uid;
      if (uid && typeof window.loadCoinsFromFirebase === 'function') {
        window.loadCoinsFromFirebase(uid);
      }
    } catch(e) {}
  })();
