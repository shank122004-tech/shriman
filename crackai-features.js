...
async function _generateQuizQuestions(exam, count, type) {
  ...
  const CF = window.CF = {
    ...
    _renderMockTest() {
      const body = document.getElementById('cf-mock-modal_body');
      if (!body) return;
      if (!window.MockTest || !window.MockTest._state) {
        // Include all study modes: exams, classes
      const exams = Object.entries(EXAM_CONFIGS).filter(([k,v])=>v.type==='exam'||v.type==='class');
        body.innerHTML = `
  ...
  function injectDOM() {
    ...
    /* ── 9. STUDY TOOLS DISABLED - Sidebar features removed ── */
    // Study tools cards (Mock Test AI, Analytics AI, Rank Predictor, Group Study AI) removed from sidebar
    // This section is intentionally commented out to keep the sidebar clean
    /*
    const drawerList = document.getElementById('historyList');
    if (drawerList && !document.getElementById('cf-sidebar-features')) {
      const items = [
        { icon:'🎯', label:'Mock Test AI',       cb:'window._openMockTestFeature()', premium:false },
        { icon:'📊', label:'Analytics AI',       cb:'window._openAnalyticsFeature()', premium:true  },
        { icon:'🏆', label:'Rank Predictor AI',  cb:'window._openScorePredictorFeature()', premium:false },
        { icon:'👥', label:'Group Study AI',     cb:'window._openGroupsFeature()', premium:false },
      ];
  ...
  function init() {
    ...
    if (window.MockTest) window.MockTest = MockTest;
  ...
  // Window-level helper functions for sidebar buttons
  window._openAnalyticsFeature = function() {
    if (typeof window.CF !== 'undefined' && typeof window.CF.openAnalytics === 'function') {
      window.CF.openAnalytics();
    } else {
      setTimeout(() => {
        if (typeof window.CF !== 'undefined' && typeof window.CF.openAnalytics === 'function') {
          window.CF.openAnalytics();
  ...
  window._openScorePredictorFeature = function() {
    if (typeof window.CF !== 'undefined' && typeof window.CF.openScorePredictor === 'function') {
      window.CF.openScorePredictor();
    } else {
      setTimeout(() => {
        if (typeof window.CF !== 'undefined' && typeof window.CF.openScorePredictor === 'function') {
          window.CF.openScorePredictor();
  ...
  window._openMockTestFeature = function() {
    if (typeof window.CF !== 'undefined' && typeof window.CF.openMockTest === 'function') {
      window.CF._showMockTestRemaining();
      window.CF.openMockTest();
    } else {
      setTimeout(() => {
        if (typeof window.CF !== 'undefined' && typeof window.CF.openMockTest === 'function') {
          window.CF._showMockTestRemaining();
  ...
  window._openGroupsFeature = function() {
    if (typeof window.CF !== 'undefined' && typeof window.CF.openStudyGroups === 'function') {
      window.CF.openStudyGroups();
    } else {
      setTimeout(() => {
        if (typeof window.CF !== 'undefined' && typeof window.CF.openStudyGroups === 'function') {
          window.CF.openStudyGroups();
