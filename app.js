...
/**
 * ✅ FIXED: Load user coins from Firestore on authentication
 * This is called when user logs in to ensure coins are loaded from
 * the authoritative source (Firestore) not localStorage
 */
async function loadUserCoinsFromFirestore(uid) {
  try {
    if (!uid) {
      console.log('[Coins] No UID provided');
      return;
    }

    if (!window._firebaseDb || !window._firebaseFns || !window._firebaseFns.doc || !window._firebaseFns.getDoc) {
      console.log('[Coins] Firebase not initialized, using localStorage');
      return;
    }

    console.log('[Coins] Loading from Firestore for UID:', uid);

    // Load existing coin record
    const coinsRef = window._firebaseFns.doc(window._firebaseDb, 'userCoins', uid);
    const coinsSnap = await window._firebaseFns.getDoc(coinsRef);

    if (coinsSnap.exists()) {
      // Found existing record - load it
      const firebaseCoins = coinsSnap.data().coins || 0;
      
      console.log('[Coins] ✅ Loaded from Firestore:', firebaseCoins);

      // Sync to localStorage
      localStorage.setItem('sscai_u:' + uid + ':coins', JSON.stringify({ coins: firebaseCoins }));
      ...
      // Update UI
      document.querySelectorAll('#prf-coin-count,#prf-shop-coin-bal').forEach(function(el) {
        if (el) el.textContent = firebaseCoins;
      });

      return firebaseCoins;
    } else {
      // First-time user - create coin document
      console.log('[Coins] Creating new coin record...');
      
      await window._firebaseFns.setDoc(coinsRef, {
        coins: 0,
        lastUpdated: new Date()
      });

      localStorage.setItem('sscai_u:' + uid + ':coins', JSON.stringify({ coins: 0 }));
      return 0;
    }
  } catch(err) {
    console.warn('[Coins] Error loading from Firestore:', err.message);
    return null;
  }
}

// Make globally available
window.loadUserCoinsFromFirestore = window.loadUserCoinsFromFirestore || loadUserCoinsFromFirestore;
...
async function sendMessage() {
  ...
  if (dom.welcomeScreen) dom.welcomeScreen.style.display = 'none !important';
  // Hide motion animation when entering workspace - call the cleanup function
  if (typeof window.hideMotionAnimation === 'function') {
    window.hideMotionAnimation();
  } else {
    // Fallback if motion animation function not available
    const motionWrapper = document.getElementById('ai-motion-wrapper');
    if (motionWrapper) {
      motionWrapper.style.display = 'none !important';
      motionWrapper.style.opacity = '0 !important';
      motionWrapper.style.visibility = 'hidden !important';
      motionWrapper.style.pointerEvents = 'none !important';
    }
  }
...
function updatePremiumUpgradeScreen() {
  try {
    ...
    // If elements aren't in the document right now, that's expected whenever
    // the user is inside an existing chat session (loadSession() detaches
    // #welcomeScreen from the DOM and doesn't re-attach it until the user
    // goes back to a new/empty session). This is NOT a sign that we need to
    // (re)start the motion animation, so we just no-op here.
    //
    // IMPORTANT: this function only ever shows/hides screens. It never
    // starts, stops, or reaches into the motion animation — that lifecycle
    // is owned entirely by professional-ai-motion.js, which manages itself
    // independently (its own premium check, DOM-ready wait, and retry loop).
    ...
    // Screen visibility is the same regardless of plan — the upgrade screen
    // stays hidden and the welcome screen stays shown. Whether the motion
    // animation appears inside #welcomeScreen instead of normal welcome
    // content is decided entirely by professional-ai-motion.js's own
    // internal premium check, not by this function.
    ...
    if (isPremium) {
      ...
      console.log('[Premium] Welcome screen shown for FREE user (motion animation is owned by professional-ai-motion.js)');
    }
  } catch(e) {
...
// Legacy payment entry point — kept only as a safe pass-through in case
// any old code still calls the bare `handlePayment(...)` identifier.
// The real implementation now lives entirely in payment.js, which sets
// window.handlePayment after this file loads. This avoids having two
// competing Cashfree checkout flows (this one used the broken
// redirectTarget:'_self' full-page redirect and a separate, unused
// 'sscai_pending_order' localStorage key).
async function handlePayment(planId) {
  if (typeof window.handlePayment === 'function' && window.handlePayment !== handlePayment) {
    return window.handlePayment(planId);
  }
  showToast('❌ Payment system not ready. Please refresh and try again.');
}
...
function loadCashfreeSDK() {
  return new Promise((resolve, reject) => {
    ...
    // Ensure PaymentJSInterface exists globally before loading SDK
    if (!window.PaymentJSInterface) {
      window.PaymentJSInterface = {
        onPaymentSuccess: function(data) {
          try { 
            if (typeof window._cashfreePaymentSuccess === 'function') {
              window._cashfreePaymentSuccess(data); 
            } 
          } catch(e) { console.error('Payment success handler error:', e); }
        },
        onPaymentFailure: function(data) {
          try { 
            if (typeof window._cashfreePaymentFailure === 'function') {
              window._cashfreePaymentFailure(data); 
            } 
          } catch(e) { console.error('Payment failure handler error:', e); }
        },
        onPaymentCancel: function(data) {
          try { 
            if (typeof window._cashfreePaymentCancel === 'function') {
              window._cashfreePaymentCancel(data); 
            } 
          } catch(e) { console.error('Payment cancel handler error:', e); }
        }
      };
    }
    ...
    script.async = true;
    script.onerror = () => {
      reject(new Error('Cashfree SDK script failed to load'));
    };
    ...
    script.onload = () => {
      ...
      const checkCashfree = setInterval(() => {
        if (typeof Cashfree === 'function') {
          ...
          clearInterval(checkCashfree);
          reject(new Error('Cashfree SDK loaded but function not available'));
        }
        attempts++;
      }, 50);
    };
...
// NOTE: window.handlePayment is intentionally NOT set here.
// payment.js (loaded after this file) provides the real, active
// implementation via window.handlePayment. This local handlePayment()
// function above is legacy/unused — kept only in case other code in
// this file still calls it internally by its local name.
...
function openAddonModal(type) {
  ...
  document.getElementById('addonPayBtn')?.addEventListener('click', async () => {
    ...
    try {
      const firebaseUser = window._firebaseAuth.currentUser;
      const orderId = 'addon_' + planId + '_' + firebaseUser.uid + '_' + Date.now();
      ...
      const res = await fetch(CASHFREE_ORDER_URL, {
        ...
        body: JSON.stringify({
          ...
          customer_name: firebaseUser.displayName || 'Student',
          customer_email: firebaseUser.email || 'student@crackai.in',
          customer_phone: (firebaseUser.phoneNumber?.replace(/\D/g,'').slice(-10)) || '9999999999',
          uid: firebaseUser.uid,
          name: firebaseUser.displayName || 'Student',
          email: firebaseUser.email || 'student@crackai.in'
        })
      });
      const data = await res.json();
      if (!data.payment_session_id) throw new Error(data.error || 'No session');
...
function openV4ProModal() {
  ...
  document.getElementById('v4ProPayBtn')?.addEventListener('click', async () => {
    ...
    try {
      const firebaseUser = window._firebaseAuth.currentUser;
      const orderId = 'addon_v4pro_' + firebaseUser.uid + '_' + Date.now();
      const token = await firebaseUser.getIdToken();
      ...
      const res = await fetch(CASHFREE_ORDER_URL, {
        ...
        body: JSON.stringify({
          order_id: orderId, 
          amount: ADDON_PRICE_V4PRO, 
          currency: 'INR',
          plan: ADDON_PLAN_V4PRO,
...
function openCompanionModal() {
  ...
  document.getElementById('companionPayBtn')?.addEventListener('click', async () => {
    ...
    try {
      const firebaseUser = window._firebaseAuth.currentUser;
      const orderId = 'addon_companion_' + firebaseUser.uid + '_' + Date.now();
      const token = await firebaseUser.getIdToken();
      ...
      if (result?.paymentDetails?.paymentMessage === 'Payment successful' || result?.error === null) {
        ...
        const vRes = await fetch(VERIFY_PAYMENT_URL, {
          ...
          body: JSON.stringify({ order_id: orderId, plan: ADDON_PLAN_COMPANION, uid: firebaseUser.uid })
        });
...
function checkAndEnforcePremiumExpiration() {
  const uid = window._firebaseAuth?.currentUser?.uid;
  if (!uid) return;
  
  const p = 'sscai_u:' + uid + ':';
  
  // Check semiannual expiration (6-month plan)
  const semiExp = localStorage.getItem(p + 'semiannual_expires');
  if (semiExp && Date.now() > parseInt(semiExp)) {
    localStorage.removeItem(p + 'premium');
    localStorage.removeItem(p + 'premium_plan');
    localStorage.removeItem(p + 'semiannual_expires');
    console.log('[Premium] 6-Month plan expired - premium access revoked');
  }
  
  // Check yearly expiration (1-year plan)
  const yearExp = localStorage.getItem(p + 'yearly_expires');
  if (yearExp && Date.now() > parseInt(yearExp)) {
    localStorage.removeItem(p + 'premium');
    localStorage.removeItem(p + 'premium_plan');
    localStorage.removeItem(p + 'yearly_expires');
    console.log('[Premium] Yearly plan expired - premium access revoked');
  }
  
  // Check if premium should be cleared based on expiry
  const premiumPlan = localStorage.getItem(p + 'premium_plan');
  if (premiumPlan === 'semiannual' && !semiExp) {
    localStorage.removeItem(p + 'premium');
  }
  if (premiumPlan === 'yearly' && !yearExp) {
    localStorage.removeItem(p + 'premium');
  }
  
  // Sync state with cleared premium
  state.isPremium = false;
  state.premiumPlan = null;
  updateUserUI();
}
...
// ===== INITIALIZATION =====
function initApp() {
  // ✅ Check premium expiration on app load
  checkAndEnforcePremiumExpiration();
  
  // ✅ Periodic premium expiration check (every 5 minutes)
  setInterval(checkAndEnforcePremiumExpiration, 5 * 60 * 1000);
...
// ═══════════════════════════════════════════════════════════════════════════════
// ✅ FIXED: Coins Persist When App Resumes - WITH RETRY LOGIC
// ═══════════════════════════════════════════════════════════════════════════════

document.addEventListener('visibilitychange', async () => {
  if (!document.hidden && state.user && state.user.uid) {
    const uid = state.user.uid;
    let success = false;
    for (let i = 1; i <= 3; i++) {
      try {
    ...
    if (success && typeof refreshAllCoinsDisplay === 'function') {
      setTimeout(() => refreshAllCoinsDisplay(), 200);
    }
  }
});
...
window.addEventListener('focus', async () => {
  if (state.user && state.user.uid) {
    try {
      await loadPremiumStatusFromFirebase(state.user.uid);
      await loadCoinsFromFirebase(state.user.uid);
      if (typeof refreshAllCoinsDisplay === 'function') refreshAllCoinsDisplay();
    } catch (e) {}
  }
});

function refreshAllCoinsDisplay() {
  const uid = state?.user?.uid;
  if (!uid) return;
  try {
    const coinsKey = 'sscai_u:' + uid + ':coins';
    const coinsData = JSON.parse(localStorage.getItem(coinsKey) || '{"coins":0}');
    const coins = coinsData.coins || 0;
    document.querySelectorAll('#drawerCoinsVal, .ba-coins-badge, [data-coins-display], #prf-coin-count, #prf-shop-coin-bal').forEach(el => {
      if (el) {
        el.textContent = el.id === 'drawerCoinsVal' ? '🪙 ' + coins + ' coins' : '🪙 ' + coins;
      }
    });
  } catch (e) {}
}

window.refreshAllCoinsDisplay = refreshAllCoinsDisplay;