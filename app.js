...
async function handlePayment(planId) {
  ...
  try {
    ...
    const orderId = `plan_${planId}_${firebaseUser.uid}_${Date.now()}`;

    // Step 1 — Call backend to create Cashfree order & get payment_session_id
    // Send all required fields expected by backend (payment.js format)
    const response = await fetch(CASHFREE_ORDER_URL, {
      ...
      body: JSON.stringify({
        ...
        currency: 'INR',
        plan: planId,
        order_note: plan.name || planId,
        customer_id: firebaseUser.uid,
        customer_name: firebaseUser.displayName || state.user.name || 'Student',
        customer_email: firebaseUser.email || state.user.email || '',
        customer_phone: (firebaseUser.phoneNumber?.replace(/\D/g,'').slice(-10)) || '9000000000',
        uid: firebaseUser.uid,
        name: firebaseUser.displayName || state.user.name || 'Student',
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      SecureLogger.error('Order creation failed:', errorData);
      showToast('❌ ' + (errorData.error || 'Payment session failed. Please try again.'));
      return;
    }
    ...
    if (!data.payment_session_id || !data.order_id) {
      SecureLogger.error('Invalid order response:', data);
    ...
    localStorage.setItem('sscai_pending_order', JSON.stringify({
      orderId: data.order_id, planId, uid: firebaseUser.uid, ts: Date.now()
    }));

    // Step 2 — Load Cashfree SDK dynamically before opening checkout
    await loadCashfreeSDK();

    // Step 3 — Initialize Cashfree and launch checkout
    ...
    // Ensure premium modal is fully visible before checkout
    const premiumModal = document.getElementById('premiumModal');
    if (premiumModal) {
      premiumModal.style.zIndex = '10000';
    ...
    cashfree.checkout({
      ...
      showToast('❌ Payment window failed to open. Please try again.');
    });

    // Step 4 — Poll for payment confirmation after redirect returns
    pollPaymentStatus(data.order_id, planId);
