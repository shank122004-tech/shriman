...
(function () {
  ...
  const CHECKOUT_URL  = 'https://us-central1-rankgpt-f8a64.cloudfunctions.net/createCheckoutSession';
  ...
  const PLANS = {
    ...
    class10:          { id: 'class10',          name: 'Class Pro (9–12)',        price: 129,  emoji: '📚', features: ['CrackAI Pro', 'Unlimited Mocks', 'Class 9-12 Content'] },
    class10_yearly:   { id: 'class10_yearly',   name: 'Class Pro Yearly',        price: 1299, emoji: '📚', features: ['CrackAI Pro', 'Unlimited Mocks', 'Class 9-12 Content'] },
  ...
  async function createOrder({ orderId, amount, planId, note }) {
    ...
    console.log('[payment.js] Order created with payment_session_id:', data.payment_session_id);
  ...
  function activatePlan(planId) {
    ...
    if (battlePlans[planId] !== undefined) {
      try {
        ...
        localStorage.setItem('sscai_u:' + uid + ':battle_tier', planId);
        localStorage.setItem('sscai_u:' + uid + ':battle_tier_expires', String(expiryTime));
        localStorage.setItem('sscai_u:' + uid + ':battle_monthly_max', String(maxBattles));
      } catch(e) {}
      syncFirestore({ battleTier: planId, battleMonthlyMax: battlePlans[planId], battleTierExpires: Date.now() + 30 * 24 * 60 * 60 * 1000 });
    }
    // ── 3c. Semiannual plan — set 6-month expiry + 1 month Battle Arena Pro ─
    if (planId === 'semiannual') {
      try {
        const uid = currentUser()?.uid || '';
        const semiExp = Date.now() + 183 * 24 * 60 * 60 * 1000; // 6 months
        const battleExp = Date.now() + 30 * 24 * 60 * 60 * 1000; // 1 month free battle pro
        localStorage.setItem('sscai_u:' + uid + ':semiannual_expires', String(semiExp));
        localStorage.setItem('sscai_u:' + uid + ':battle_tier', 'battle_pro');
        localStorage.setItem('sscai_u:' + uid + ':battle_tier_expires', String(battleExp));
        localStorage.setItem('sscai_u:' + uid + ':battle_monthly_max', '19');
      } catch(e) {}
      syncFirestore({ semiannualExpires: Date.now() + 183 * 24 * 60 * 60 * 1000, battleTier: 'battle_pro', battleMonthlyMax: 19, battleTierExpires: Date.now() + 30 * 24 * 60 * 60 * 1000 });
    }
    ...
    if (planId === 'yearly') {
      try {
        const uid = currentUser()?.uid || '';
        const yearExp = Date.now() + 365 * 24 * 60 * 60 * 1000; // 12 months
        const battleExp = Date.now() + 60 * 24 * 60 * 60 * 1000; // 2 months free battle pro
        localStorage.setItem('sscai_u:' + uid + ':yearly_expires', String(yearExp));
        localStorage.setItem('sscai_u:' + uid + ':battle_tier', 'battle_pro');
        localStorage.setItem('sscai_u:' + uid + ':battle_tier_expires', String(battleExp));
        localStorage.setItem('sscai_u:' + uid + ':battle_monthly_max', '19');
      } catch(e) {}
      syncFirestore({ yearlyExpires: Date.now() + 365 * 24 * 60 * 60 * 1000, battleTier: 'battle_pro', battleMonthlyMax: 19, battleTierExpires: Date.now() + 60 * 24 * 60 * 60 * 1000 });
    }
  ...
  /* ✅ NEW: Plan Expiration Checking Functions */
  window.checkPlanExpiration = function(planId) {
    ...
    const expiryKeys = {
      'semiannual': 'sscai_u:' + uid + ':semiannual_expires',
      'yearly': 'sscai_u:' + uid + ':yearly_expires',
      'battle': 'sscai_u:' + uid + ':battle_tier_expires',
      'battle_pro': 'sscai_u:' + uid + ':battle_tier_expires',
      'battle_academy': 'sscai_u:' + uid + ':battle_tier_expires'
    };
    
    const expiryTime = localStorage.getItem(expiryKeys[planId]);
    if (!expiryTime) return false; // Not purchased
    ...
    if (expired) {
      // Clear expired plan
      localStorage.removeItem(expiryKeys[planId]);
      localStorage.removeItem('sscai_u:' + uid + ':' + planId);
      localStorage.removeItem('sscai_u:' + uid + ':battle_tier');
      localStorage.removeItem('sscai_u:' + uid + ':battle_monthly_max');
      console.log('[Payment] Plan ' + planId + ' has expired');
      return false;
    }
  ...
  window.getActiveBattlePlan = function() {
    const uid = (typeof window._firebaseAuth !== 'undefined' && window._firebaseAuth.currentUser) ? window._firebaseAuth.currentUser.uid : null;
    if (!uid) return null;
    
    const battleTier = localStorage.getItem('sscai_u:' + uid + ':battle_tier');
    if (!battleTier) return null;
    
    // Check if expired
    const expiryTime = localStorage.getItem('sscai_u:' + uid + ':battle_tier_expires');
    ...
    return {
      tier: battleTier,
      limit: parseInt(localStorage.getItem('sscai_u:' + uid + ':battle_monthly_max') || '0'),
      expiresAt: expiryTime ? parseInt(expiryTime) : null
    };
  };

  window.getRemainingDaysForPlan = function(planId) {
    const uid = (typeof window._firebaseAuth !== 'undefined' && window._firebaseAuth.currentUser) ? window._firebaseAuth.currentUser.uid : null;
    if (!uid) return 0;
