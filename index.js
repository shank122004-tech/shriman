...
// Creates Cashfree checkout session after order is created
exports.createCheckoutSession = onRequest((req, res) => {
  ...
  cors(req, res, async () => {
    ...
    try {
      ...
      functions.logger.info('[createCheckoutSession] received', {
        order_id,
        payment_session_id,
        uid: decoded.uid,
      });
      ...
      if (!payment_session_id) {
        return res.status(400).json({ error: 'payment_session_id required' });
      }

      const sessionRes = await axios.post(
        `${CF_API}/view/sessions/checkout`,
        {
          order_id: String(order_id),
          payment_session_id: String(payment_session_id),
        },
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      functions.logger.info('[createCheckoutSession] success', {
        session_id: sessionRes.data.session_id,
      });

      return res.json({
        session_id: sessionRes.data.session_id,
        order_id: sessionRes.data.order_id,
      });
      ...
      functions.logger.error('[createCheckoutSession] error', {
        status,
        message: cfErr?.message || err.message,
      });

      return res.status(status).json({
        error: cfErr?.message || err.message || 'Checkout session failed',
      });
    }
  });
});
